...
export default function ProjectPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  // Recursive helper to add files/folders to ZIP
  const addFilesToZip = (zip: JSZip, nodes: FileNode[]) => {
    nodes.forEach((node) => {
      if (node.type === 'file') {
        ...
        if (folder) addFilesToZip(folder, node.children)
      }
    })
  }
  ...
  const downloadProject = () => {
    ...
    if (project.files) addFilesToZip(zip, project.files)
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          ...
          <main className="lg:col-span-3 bg-gray-950 border border-gray-800 rounded-xl p-6">
            <div className="flex items-center justify-between mb-4 gap-2 flex-wrap">
              <h2 className="text-cyan-300 text-lg flex items-center gap-2">🧑‍💻 Project Code</h2>
              ...
              <div className="flex gap-2 flex-wrap">
                ...
                <button
                  ...
                  className="inline-flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white font-semibold px-4 py-2 rounded-md"
                >
                  💾 Download Project
                </button>
              </div>
